...
<div align = "left">
  ...
  <a href="https://catnip-puppy-52c.notion.site/0f30ccd74be94f7cb0b00c4bca706361?pvs=4">정리 노션</a>ㅣ<a href="https://github.com/SolPict/SolPict-server">서버</a>
</div>
...
<!-- toc -->
  ...
  * [사전 설명](#%EC%82%AC%EC%A0%84-%EC%84%A4%EB%AA%85)
    ...
    + [기존 AI의 수학 문제 해결 과정은 어떤 방식인가?](#%EA%B8%B0%EC%A1%B4-ai%EC%9D%98-%EC%88%98%ED%95%99-%EB%AC%B8%EC%A0%9C-%ED%95%B4%EA%B2%B0-%EA%B3%BC%EC%A0%95%EC%9D%80-%EC%96%B4%EB%96%A4-%EB%B0%A9%EC%8B%9D%EC%9D%B8%EA%B0%80)
- [문제, 해결방안](#%EB%AC%B8%EC%A0%9C-%ED%95%B4%EA%B2%B0%EB%B0%A9%EC%95%88)
  ...
  * [3. 딥러닝 모델 적용 후 도전과제](#3-%EB%94%A5%EB%9F%AC%EB%8B%9D-%EB%AA%A8%EB%8D%B8-%EC%A0%81%EC%9A%A9-%ED%9B%84-%EB%8F%84%EC%A0%84%EA%B3%BC%EC%A0%9C)
    ...
    + [3-2. 고성능 GPU 없는 노트북 개발 환경에서의 한계](#3-2-%EA%B3%A0%EC%84%B1%EB%8A%A5-gpu-%EC%97%86%EB%8A%94-%EB%85%B8%ED%8A%B8%EB%B6%81-%EA%B0%9C%EB%B0%9C-%ED%99%98%EA%B2%BD%EC%97%90%EC%84%9C%EC%9D%98-%ED%95%9C%EA%B3%84)
...
<div align="left">
  <img width="700" alt="image-1" src="https://gist.github.com/user-attachments/assets/08e60f0d-9a73-4378-98f4-ef6082c12742">   
</div>
...
- **모바일 ✓** vs 웹
  ...
  1. 프로젝트의 메인기능인 이미지를 찍어서 문제를 해결하는 과정을 거치므로 유저입장에서 쉽게 이미지를 찍을 수 있는 **모바일** 기술을 선택하였습니다.
  2. 모바일일 경우 좀 더 유연한 환경에서 서비스를 제공 받을 수 있기 때문에 사용자 경험을 극대화 하기위해 **모바일** 기술을 선택하였습니다.

- **Expo ✓** vs Cli

  1. 짧은 프로젝트 기한 동안 여러가지를 구현해야 하는 입장으로써 다양한 API 와 Expo Go 라는 앱을 제공해주어 개발을 용이하게 도와주기 때문에 **Expo**를 선택하였습니다.
  2. 모바일 플랫폼 환경에서 처음으로 개발해보기 때문에 초기셋팅과 빌드 및 배포 그리고 학습적인 측면에서 좀 더 간편하기 때문에
     **Expo**를 선택하였습니다.

- **Python ✓** vs Javascript

  1. 프로젝트에서 딥러닝 모델을 사용해야 해야해서 pytorch 또는 TensorFlow가 꼭 필요한 상황이였기 때문에 **Python**언어를 선택하였습니다.
  2. 그 외에도 수식 관련 다룰 수 있는 SymPy, 이미지 처리(OCR) 관련해서도 OpenCV, EasyOCR 등 데이터를 조작하고 다룰 수 있게 도와주는 라이브러리들이 많았기 때문에 **Python**언어를 선택하였습니다.

- **S3 ✓**
  1. 기존 데이터베이스(MongoDB)가 있지만 프로젝트 특성상 이미지라는 용량이 큰 파일을 다루기 때문에 대용량의 데이터를 다루기 용이한 **S3**3를 선택하였습니다.
  2. 클라이언트 측에서도 쉽게 접근할 수 있는 기능을 제공해주기 떄문에 서버의 부담을 줄일 수 있고 요청 또한 서버와 병렬적으로 보내서 작업할 수 있기 때문에 **S3**을 선택하였습니다.
...
AI와 사람을 비교했을 때, 사람은 창의적이고 논리적인 사고를 통해 수학 문제를 해결합니다. 반면 AI는 대규모 데이터를 기반으로 규칙을 찾아 결과를 추론하고 예측하는 방식으로 문제를 접근하므로, “사고”를 한다고 볼 수 없습니다.

또한, 사람은 수학 문제를 해결할 때 수식을 세우고 다양한 방식으로 변형하며 창의적으로 접근할 수 있습니다. 반면, AI는 정해진 방식과 특정 패턴에 따라 정답을 도출하는 데 그치기 때문에 이러한 유연성과 창의성을 발휘하는 데 한계가 있습니다.

그러면 어떻게 수학문제를 풀 수 있게 할 수 있을까요?

### 기존 AI의 수학 문제 해결 과정은 어떤 방식인가?

기존 존재하는 대부분의 서비스에서 AI가 수학문제를 풀 수 있는 것의 대한 해답은 **문제 전체** 를 구조화된 코드 형태로 바꾸는 것에 있습니다.

구조화된 형태로 변환된 수학 문제는 딥러닝 AI에 의해 큰 문제 유형으로 먼저 분류됩니다. 그 후, 세부적으로 다시 분류되어 각 유형에 맞는 풀이 과정을 거칩니다. 문제 해결 과정과 정답은 기록되어, 마지막으로 자연어 생성(NLG) 과정을 통해 사용자에게 풀이 과정과 함께 결과가 제공됩니다.
...
이미지 분석에 있어서 정확도를 Confident로 나타낼 수 있는데,
기존에 사용하려던 이미지 분석(OCR) 기술은 텍스트에 대해서는 비교적 정확하게 분석하는 것을 확인할 수 있었습니다. 텍스트 분석 결과의 신뢰도는 Confident 1에 가까웠습니다. 하지만, 수식에 대한 분석은 정확도가 떨어지며, 신뢰도가 Confident 0에 가까운 결과를 보여줍니다.
...
그렇게 저는 두 가지 방법을 시도했습니다.

<INS>첫 번째 방법</INS>은 OpenCV를 사용하여 수식 영역과 텍스트 영역을 분리한 후, 각각 따로 분석하는 방식이었습니다.  
그러나 이 방법에서는 영역 분리가 제대로 이루어지지 않았고, 학습 곡선이 가파르다는 문제로 인해 기대했던 성과를 얻지 못했습니다. 결과적으로, 아래와 같은 부실한 분석 결과가 도출되었습니다.
<br>
...
<INS>두 번쨰 방법</INS>은 여러 수식 전용 외부 API 서비스를 비교한 후, 가장 정확하게 수식을 분석해주는 API를 선택하는 것이었습니다.  
...
<INS>첫째,</INS> 딥러닝 모델을 직접 설계하고 학습시키기 위한 관련 지식이 부족했기 때문입니다.
...
딥러닝 모델이 좋은 성능을 발휘하려면 많은 양의 학습 데이터가 필요합니다. 특히, 수학 문제 풀이와 관련된 모델의 경우 문제 유형에 따른 다양한 데이터를 확보해야 하는데, 이는 수학적 개념, 문제의 난이도, 문제 유형에 따라 달라질 수 있습니다. 일반적으로 모델을 제대로 학습시키려면 최소 2,000개 이상의 데이터가 필요하기 때문에 많은 양의 데이터를 확복하기 어렵기 때문에 학습된 모델을 활용하는 방법을 선택했습니다.
...
<INS>첫번째 이유는</INS> 대규모 딥러닝 모델(LLM)이 연산을 수행하는 데 필요한 대량의 행렬 계산입니다.  
이러한 계산은 CPU로 처리할 경우 상당한 시간이 소요되어 성능 저하를 초래할 수 있습니다. 반면, GPU는 병렬 작업에 특화되어 있어 동시에 많은 계산을 수행할 수 있어 연산 시간을 획기적으로 단축할 수 있습니다.그렇기 때문에 수학 문제를 해결하는 데 필요한 복잡한 수식과 행렬 계산을 GPU를 사용하면 훨씬 더 빠르게 처리할 수 있습니다.

<img width="700" alt="image" src="https://github.com/user-attachments/assets/369f102a-ab9a-43b2-89a4-8a9f79f7decc">

<br>

<INS>두번째 이유</INS>는 대규모 딥러닝 모델(LLM)을 학습하고 훈련하는 과정에서 조정해야 하는 엄청난 수의 매개변수입니다.  
이러한 매개변수 조정은 병렬적으로 수행해야 하며, GPU의 병렬 처리 능력을 활용하여 훈련 시간을 크게 단축할 수 있습니다. 이는 특히 많은 데이터셋을 다루거나 복잡한 알고리즘을 적용할 때 필수적입니다.

따라서, 수학 문제를 풀어주고 계산 과정을 저장할 수 있는 대규모 모델을 구축하기 위해서는 GPU가 필수적입니다. 이러한 GPU는 연산과 훈련 과정에서 요구되는 성능을 제공하여, 더 나은 결과를 얻고 개발 과정을 효율적으로 진행할 수 있도록 도와줍니다.

### 3-2. 고성능 GPU 없는 노트북 개발 환경에서의 한계

딥러닝 모델을 사용하기 위해서는 고성능 GPU가 반드시 필요했지만, 제 개발 환경은 노트북이라 충분한 GPU 성능을 제공하지 못했습니다.
그래서 이를 해결하기 위해 두 가지 방법을 활용해서 개발을 진행하였습니다.

<INS>첫 번째 방법으로는</INS>, Google에서 제공하는 무료 클라우드 기반 Python 개발 환경인 ‘Colab’을 활용했습니다.  
Colab은 고성능 GPU를 제공하므로, 이를 통해 딥러닝 모델을 실행하고, 입력에 따른 출력을 받아와서 사용할 수 있었습니다. 이렇게 클라우드 환경을 이용하면 복잡한 딥러닝 모델을 노트북에서도 손쉽게 사용할 수 있습니다.
...
이 방법을 통해 집에 있는 데스크탑 컴퓨터를 서버로 활용할 수 있었습니다. ngrok은 로컬에서 실행 중인 서버를 안전한 터널을 통해 인터넷에 공개할 수 있게 해주며, 이를 통해 외부 어디서든 로컬 서버에 접근할 수 있습니다. 이렇게 데스크탑의 성능을 활용하여 더 안정적인 환경에서 모델을 실행할 수 있었습니다.

 <img width="700" alt="image" src="https://github.com/user-attachments/assets/8e44e336-7c52-444d-a9a5-0feac95c631b">

 <br>

위의 방법들을 통해 고성능 GPU의 부족 문제를 해결하면서, 효율적으로 딥러닝 모델을 활용할 수 있었습니다. Colab은 빠른 개발과 테스트에 유용했으며, ngrok을 이용한 로컬 서버는 지속적으로 서버를 운영하고 외부에서 접근이 필요할 때 적합한 솔루션이 되었습니다.

## 4. 딥러닝 모델 정확도 향상 전략

### 4-1. 이미지 분석(OCR) 정확도 향상을 통한 방법

이미지 분석(OCR)을 통한 정확도를 향상시키기 위해 저는 <INS>2가지 방법</INS>을 사용하였습니다.

<INS>첫 번째 방법으로</INS>,보다 높은 정확도를 제공하는 외부 API 서비스로 변경했습니다.  
기존 서비스는 원본 사진이 조금만 어둡거나, 숫자 인식에서 오류가 발생하는 경우가 많았습니다. 하지만 새로운 API를 사용하면서 이러한 문제들이 크게 개선되었습니다.
...
위와 같이, 원본 사진이 다소 어둡거나 번호 인식에 문제가 있었던 부분이 개선된 것을 확인할 수 있습니다. 더 나은 인식 성능을 제공하는 API를 통해 텍스트 추출의 정확도가 크게 향상되었습니다.
...
위 사진처럼, 이미지 자르기를 통해 불필요한 부분을 제거하여 OCR 처리 시 중요한 부분만 인식할 수 있도록 했습니다. 또한, 기울어진 이미지를 회전 기능으로 보정해, 인식 대상이 똑바로 정렬되도록 하여 인식의 정확도를 더욱 높였습니다. 이러한 전처리 과정은 OCR 엔진이 텍스트를 보다 정확하게 추출할 수 있게 해주었습니다.

추가로, 더 나은 정확도를 위해 직접 이미지 분석(OCR) 기술을 배우고 활용하는 것도 고려해볼 수 있습니다. 이 방법은 이미지 전처리와 후처리를 더욱 세밀하게 다룰 수 있게 해주며, 특정 문제에 맞춤형으로 OCR 성능을 최적화할 수 있는 가능성을 제공합니다. 예를 들어, 텍스트 추출 전 이미지의 노이즈를 제거하거나, 특정 문자 패턴에 맞게 조정하는 등의 작업을 수행할 수 있습니다. 이를 통해 더 정밀한 OCR 처리 결과를 얻을 수 있을 것입니다.
...
|                                                          한글로 입력                                                          |                                                          영어로 입력                                                          |
| :---------------------------------------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------------: |
| <img width="450" height="" alt="image" src="https://github.com/user-attachments/assets/2ab41ad5-6658-4451-8e71-414baf412b14"> | <img width="380" height="" alt="image" src="https://github.com/user-attachments/assets/3071af51-cba4-4224-912e-4070eba595de"> |

결과를 보면, 한글로 입력했을 때는 "5"라는 예상치 못한 답이 나오는 것을 볼 수 있습니다. 반면, 영어로 입력했을 때는 정상적으로 "0.5 (1/2)"라는 답이 도출되었습니다. 이는 모델이 영어 데이터로 학습된 경우, 영어로 입력할 때 더 정확한 결과를 제공할 수 있다는 것을 보여줍니다.
...
| **2주차** | - Expo 이용한 새로운 플랫폼(모바일) 정적페이지 구현<br> - 이미지 분석(OCR) 기술 구현 가능 여부 판단(조사, 시도) -> API 사용으로 결론<br> - 딥러닝 모델 구현 가능 여부 판단(조사, 시도) -> 학습된 딥러닝 모델 가져오기로 결론 |
| **3주차** | - 프로젝트에 적합한 이미지 분석(OCR) API 탐색 및 적용<br> - 프로젝트에 적합한 딥러닝 모델 탐색 및 적용                                                                                                                       |
| **4주차** | - 새로운 환경(FastAPI) 서버 CRUD 구현<br> - 클라이언트측 UI, UX 구현<br> - GPU 부재로 인한 집에서 서버 연결(조사, 시도)<br> - 상호연결에 의한 에러 처리                                                                      |

# 프로젝트 소감

짧았던 프로젝트 기간에 비해 프로젝트 규모가 컸던 만큼, 전체적인 구현을 시간 내에 완료하는 것이 매우 중요했습니다. 프로젝트를 진행하면서 다양한 시도를 해보았고, 잘되지 않았을 경우 해결 방안에 대해 많은 고민을 하였습니다.

특히 OCR과 딥러닝에 대한 사전 지식이 부족했기 때문에 외부 API나 학습된 모델 없이 이러한 기술들을 구현할 수 있을지에 대한 판단이 불확실했습니다. 이로 인해 시간 관리에 많은 어려움을 겪었고, 어려움에 직면할 때마다 어떤 선택을 해야지 프로젝트를 완성에 가까이 이끌 수 있을지에 대한 고민을 하며 선택했던 것 같습니다.

또한, 익숙하지 않은 모바일 플랫폼 환경과 새로운 언어인 Python은 “내가 과연 잘 할 수 있을까?”라는 걱정을 안겨주었지만, 동시에 새로운 기술을 배운다는 기대감도 함께했습니다. 덕분에 프로젝트를 진행하면서 새로운 기술을 배우고 사용하는 데 큰 재미와 즐거움을 느낄 수 있었습니다.
